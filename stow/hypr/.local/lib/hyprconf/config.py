...
def _hyprctl_to_section_key(hkey: str) -> tuple[str, str]:
    """Reverse of section_key_to_hyprctl: "decoration:blur:enabled" -> ("decoration.blur", "enabled")."""
    parts = hkey.split(":")
...
def _build_nested_config(managed: dict[str, str]) -> dict[str, object]:
    ...
    for hkey in sorted(managed):
        section, key = _hyprctl_to_section_key(hkey)
...
def _parse_flat_lines(body: str) -> dict[str, str]:
    ...
    for ln in body.splitlines():
        ...
        if m:
            managed[m.group(1).strip()] = m.group(2).strip()
    return managed
...
def read_persisted(section: str, key: str) -> str | None:
    ...
    if marker is None:
        return None
    return _flatten_config_call(body).get(hkey)
