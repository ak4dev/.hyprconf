...
THEME_COLORS_CONF = os.path.expanduser("~/.config/hypr/theme-colors.lua")
...
def update_hyprland_borders(theme: dict[str, str]) -> None:
    """Write theme-colors.lua and update Hyprland border colors live via hyprctl."""
    ...
    accent_rgba = hex_to_hypr_rgba(accent)
    secondary_rgba = hex_to_hypr_rgba(secondary)
    # `hyprctl keyword` (below) uses hyprlang's classic "color1 color2 ANGLEdeg"
    # wire form regardless of config file format.
    active_border = f"{accent_rgba} {secondary_rgba} 45deg"
    inactive_border = hex_to_hypr_rgba(inactive, "aa")

    # Write the require()d Lua module for persistence across reloads and
    # restarts. Hyprland 0.55+ moved the compositor config to Lua (hyprlang
    # `.conf` deprecated 0.56, removal ~0.57) — see docs/hyprland-reference.md.
    # The Lua API rejects the combined "color1 color2 ANGLEdeg" string form
    # for a gradient (verified against a real Hyprland binary via
    # `Hyprland --verify-config`) — it needs the `{ colors = {...}, angle = }`
    # table form instead.
    content = (
        ...
        "    general = {\n"
        "        col = {\n"
        f'            active_border = {{ colors = {{ "{accent_rgba}", "{secondary_rgba}" }}, angle = 45 }},\n'
    ...
    try:
        ...
        print(f"Warning: could not write theme-colors.lua: {e}")
