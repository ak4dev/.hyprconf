...
- **Avoid the AUR at all costs — and never install from it without asking first.** Official repositories only. Every package this project installs automatically, on any code path (`setup.sh`, the Omarchy overlay under `omarchy/`, or anything added later), must come from an official Arch repository. If a feature appears to require an AUR package, **stop and raise it with the user, and verify the package, before implementing anything** — do not add it, do not add an AUR helper call, and do not quietly pick an AUR-only dependency to make a feature work. AUR packages remain something the user installs manually, by choice.
  ...
  This binds the Omarchy overlay too, and deliberately so: Omarchy ships `yay`, uses the AUR itself, and exposes `omarchy-pkg-aur-add`. Its availability is not permission to use it — hyprconf does not reach for the AUR just because the host platform does. Enforced by `tests/unit/test_no_aur_policy.py` and by the overlay scans in `tests/unit/test_omarchy_install.py` (no `yay`, no `makepkg`, no `omarchy-pkg-aur-add`).
...
- **`hc` (`stow/hypr/.local/bin/hc`) is a pure-forwarding alias table — never let it become a dispatcher.** Each arm maps a short name (`vm`, `gpu`, `secureboot`, `vpn`, `power`, `yubikey`, `theme`, `sync`) straight to the real standalone tool and hands off the remaining args untouched (`shift; exec <tool> "$@"`). It must never parse, validate, or interpret those args, and must never grow its own per-subcommand `--help` or flag handling — this repo already built and removed a ~2,300-line CLI dispatcher for exactly that reason (commit `fbbd13f`: unused in favor of the TUI). Adding a new standalone tool gets exactly one forwarding line in `hc`; any argument logic belongs in the tool itself, not here.
