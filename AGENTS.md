...
- A lean overlay on a stock [Omarchy](https://omarchy.org): `install.sh` runs every `modules/*/install` — self-contained, idempotent, order-free, each with its own `install undo`, README, tests and payload — through Omarchy's own seams (map below). It is the only entry point; with no payload beside it (the curl path) it clones `stable` and hands over. Nothing is packaged: the promoted branch is the release.
- Verified against Omarchy 4.0.3-1 (Hyprland 0.56.2, quickshell 0.3.1, uwsm 0.26.7, Firefox 155.0.1-1, git 2.55, Bash 5.3). This line is the pin; a version in a code comment or a test is that fact's own provenance and stays as written.
- A personal config shared as-is, not a product; no `LICENSE`, deliberately. Favour deleting over adding: every line ships into a user's `$HOME`. `web/` is one static page: no build, no JS, no external requests.
...
| [bar-resources](modules/bar-resources/README.md) | as above, plus `kinds: ["service"]` behind `PluginShellApi`, once per session |
...
- The greeting at `~/.config/hyprconf/fastfetch.jsonc`, not fastfetch's user slot: `~/.config/fastfetch/config.jsonc` is what Omarchy's About screen renders (skipping its window fit whenever the file exists), so that slot stays Omarchy's.

## The Hyprland copies

`modules/hypr/{bindings,input,looknfeel}.lua` become `~/.config/hypr/*.lua` by copy, so an edit is live only after `hyprconf hypr`. Every save must parse (`luac -p modules/hypr/*.lua`), state only deltas from `/usr/share/omarchy/default/hypr/`, and bind only commands already on PATH (`command -v` first: `test_hypr.py` covers the `hyprconf-*` half only, and Hyprland runs a missing target as a silent no-op). `mainMod` is `SUPER`; every key Omarchy also binds goes through `rebind()`, which unbinds the keysym and the keycode form before `o.bind`; never restate a bind Omarchy already has (`SUPER+P`, arrows, scroll, mouse drag). A program outside Omarchy's base goes in the owning module's `packages`. A new preset is one more `*Monitors*.lua` file (the seed and `-h` derive from the glob), a README row and a `test_hypr.py` case; desk presets are `desc:`-keyed, serial-free and end on the `output = ""` catch-all (pinned there).
...
- Runtime Hyprland changes go through `hyprctl eval` and `hyprctl dispatch 'hl.dsp.…'` (quirks). A hotkey-launched script speaks through `omarchy-osd` / `omarchy-notification-send` or not at all. No `pkill` / `killall`.
...
- Omarchy: the installed tree, `omarchy commands --json`, `omarchy <group> --help`, the skill it ships at `$OMARCHY_PATH/default/agents/skills/omarchy/` (SKILL.md + hyprland/plugins/theming/hooks); <https://omarchy.org/manual/> for user-facing behaviour.
...
`dev` is the working branch; `stable` is what users clone and `hyprconf.sh` serves. Only `scripts/publish` writes `stable` (the three gates, tag `v$(<VERSION)`, one atomic push of `dev`, `stable` and the tag), only when the user says "publish", with CI green on `dev`. Never push unless the user asks; commit when asked. Nothing in the repo deploys: the website upload is by hand, only when the user says "deploy", after a publish, always `stable`'s `install.sh` (CONTRIBUTING › Updating the website).

## Commits and versions

[Conventional Commits](https://www.conventionalcommits.org/): `<tag>(<scope>): <message>`, subject ≤ 140 chars; tags `feat fix docs style refactor perf test build ci chore revert`; `!` or a `BREAKING CHANGE:` footer for a removed or renamed hotkey, module, flag, plugin id or path; name the Omarchy version verified against. [SemVer](https://semver.org/) in `VERSION`, bumped by hand in the commit that earns it (minor for a `feat`, major for a break); `scripts/publish` reads it, takes no options, and refuses a tag already on another commit.

## Tests

- One hermetic suite in two places, `modules/<name>/test_<name>.py` beside each `install` and `tests/` (the core, the scans, the plugin contract, PII, the supply chain, publish, the docs contract), collected in one run; what needs a live session is verified by hand and recorded in the commit.
- Hermetic: green in a bare `archlinux:latest` container as an unprivileged user — no Hyprland, shell, hardware, real `$HOME` or ambient state, never mutating the box; there is no root axis (`install.sh` refuses root, CI runs unprivileged). Every command that could touch the desktop or the system is a fake first on PATH; pure tools (`jq`, `luac`, `qmllint`, `shellcheck`, `zsh`) are real when present, skipped otherwise; `git` is required (throwaway trees under `tmp_path` only).
- Harness: the repo-root `conftest.py`'s `box` fixture — a throwaway machine with a recording fake for every `omarchy-*` name the tree carries, derived from it — which also exports the session's git identity, so no test passes `-c user.*`. New tests use it; its API and the fakes: CONTRIBUTING › Writing hermetic tests.

## Gates and CI

Before every commit: `make check` = `make lint` (ruff check + format) + `make shellcheck` (by shebang; fails on finding nothing) + `make test` (`pytest -n auto -rs`), the gates `scripts/publish` runs. `.github/workflows/test.yml` runs the same on every push, in `archlinux:latest`, as an unprivileged `ci` user it creates. Exactly seven tests skip there, each because it needs the installed Omarchy (the four `omarchy-plugin-validate` runs, `bar-clock`'s two parity tests, `firefox`'s merge with Omarchy's `policies.json`); any other skip is a regression, and `-rs` names each; `OMARCHY_PATH=$(mktemp -d) make test` reproduces exactly those seven here, since every such probe keys on that one seam. Read a red run from its `::error::` annotation, the one part the public API hands back (why: the comment at the workflow's tee step). Reproduce a container-only failure with:
...
- `hyprctl keyword` is a no-op under the Lua parser and `dispatch dpms on` errors: use `hyprctl eval` (exit 7 on error) and `hyprctl dispatch 'hl.dsp.…({ … })'`; forms in `modules/hypr/README.md`.
- Omarchy binds digits and `-`/`=` by keycode (`SUPER + SHIFT + code:20`, `default/hypr/bindings/tiling.lua`), which `hl.unbind` of the keysym does not match; the keycode unbind inside `rebind()` is load-bearing.
...
- A symlinked plugin folder is scanned (`shell/services/PluginRegistry.qml:712-713`) but never watched (`inotifywait -r`, `:663-674`): the explicit `rescanPlugins` on every run is what picks a `git pull` up. Bar widgets never size off `parent` (a binding loop QML drops): bind `implicitWidth` / `implicitHeight` to content.
...
Read this file, the module README and the comments beside what you touch; verify every Omarchy fact on the box first (rule 2); change code, tests and docs together and run `make check` before every commit (CI on the push is the container gate); commit when asked, and never push, publish or deploy unless the user says so, in those words. `.claude/settings.json`'s deny/ask rules back that line and rules 2–3: a speed bump matching command text, not a boundary.
