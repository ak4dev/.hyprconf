...
| Default browser | `omarchy-default-browser firefox`, **once** (marker `browser-applied`; the v7 `defaults-applied` marker is honoured for one release). The value is read back, never the setter's exit status — that status is its closing notification's |
...
`HYPRCONF_NO_SUDO` (what `--no-packages` and the post-update hook set) or no terminal: the install and the policy are skipped with a pointer line — the default-browser seed still runs, because on a settled box the hook's run is the only one there is. `HYPRCONF_STATE` moves the marker directory.
