...
- Installs **VS Code** when missing, through Omarchy's own `omarchy-install-editor-vscode`:
  `visual-studio-code-bin` from Omarchy's `[omarchy]` repository (never the AUR), `~/.vscode/argv.json`,
...
`defaults-applied` — one marker for browser and editor both — counts as applied for one release, delete
both to seed again. The default itself lives in `~/.local/state/omarchy/defaults/editor`.

## Undo
`bash modules/vscode/install undo` — the default editor returns to Omarchy's stock `nvim` if it is still
`code` (a pick made after the install stays); the marker goes; VS Code stays installed
