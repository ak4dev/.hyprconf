...
def test_upsert_and_read_persisted(hypr_dir: Path) -> None:
    from hyprconf.config import read_persisted, upsert_option
...
def test_save_pending_multi_section(hypr_dir: Path) -> None:
    from hyprconf.config import read_all_persisted, save_pending
...
def test_overrides_file_contains_managed_marker(hypr_dir: Path) -> None:
    from hyprconf.config import MANAGED_MARKER, OVERRIDES_FILE, upsert_option
...
def test_get_option_returns_string_value(hypr_dir: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_set_option_calls_hyprctl(hypr_dir: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("HYPRLAND_INSTANCE_SIGNATURE", "test")
    from hyprconf import hyprctl

    with patch("subprocess.run", return_value=_mock_run(0, "ok")) as mock_run:
        hyprctl.set_option("general", "gaps_in", "12")
...
def test_add_keybind_produces_correct_format(hypr_dir: Path) -> None:
    from hyprconf.keybinds import KEYBINDS_FILE, add_keybind
...
def test_upsert_monitor_writes_correct_line(hypr_dir: Path) -> None:
    from hyprconf.monitors import MONITORS_FILE, upsert_monitor
...
def test_upsert_monitor_with_extras(hypr_dir: Path) -> None:
    ...
    upsert_monitor("HDMI-A-1", "preferred", "auto", "1", "vrr, 1, bitdepth, 10", file=MONITORS_FILE)
    text = MONITORS_FILE.read_text()
