...
def hypr_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    # These are Omarchy's ~/.config/hypr override points (Lua, Hyprland 0.55+):
    # bindings.lua is where keybinds live; monitors.lua is the active preset.
    (hypr / "bindings.lua").write_text('local mainMod = "SUPER"\n')
    ...
    monkeypatch.setattr(_keybinds_mod, "KEYBINDS_FILE", hypr / "bindings.lua")
