...
The tool enrols a key into the LUKS header, rewrites the initramfs hook set
and the kernel cmdline, and rebuilds the boot images — so the real sudo,
systemd-cryptenroll, limine-mkinitcpio and omarchy-snapshot would do all of
that to the developer's own machine. Every external is therefore a recording
stub first on PATH (the `box` fixture in conftest.py, with the bodies below
over it) and every system path an env seam pointed at a tmp copy
(CONTRIBUTING › Writing hermetic tests). stdin is never a terminal here, so
the --yes / TTY guard is exercised on every run that does not ask for one.
"""
...
# (/etc/mkinitcpio.conf.d/omarchy_hooks.conf; Omarchy's own second copy of it
# is default/hypr/input.lua:24-27). Neither is under $OMARCHY_PATH in a shape
# a suite that must pass without Omarchy could read, so it is copied here and
# pinned against the tool's case line below.
...
# The bodies this tool's externals need over the box's recording stubs;
# limine-update is deliberately absent — the rebuild is limine-mkinitcpio, the
# tool Omarchy runs after its own drop-in changes (omarchy-hibernation-setup
# says why limine-update is the wrong one here: it would re-deploy the
# bootloader binary and build a second time), so a call to it finds no fake
...
@pytest.fixture
def box(box: Box) -> Box:
    """The box fixture (conftest.py) made into a throwaway Omarchy: limine's
    config files, a mkinitcpio.conf.d, vconsole.conf, one installed kernel, a
    machine-id, a UEFI marker and an ESP, each behind the tool's own
    _HYPRCONF_* seam, and this tool's externals given bodies over the box's
    recording stubs.

    Stock Omarchy 4.0.0 shape, unchanged in 4.0.3-1: ESP_PATH in
    /etc/default/limine, ENABLE_UKI=yes from
    /etc/limine-entry-tool.d/omarchy-uki.conf over the packaged
    ...
    The paths and the fakes' knobs hang on the box as plain attributes; the
    knobs are read at run() time, so a test can set them after setup.
    """
    box.state = box.tmp / "state"
    box.state.mkdir()
    box.esp = box.tmp / "boot"
    box.esp.mkdir()
    ...
    box.entry_conf = box.etc / "limine-entry-tool.conf"
    box.entry_conf.write_text("ENABLE_VERIFICATION=yes\nENABLE_UKI=no\n")
    ...
    (box.conf_d / "omarchy-uki.conf").write_text("ENABLE_UKI=yes\n")
    box.omarchy_conf_d = sorted(p.name for p in box.conf_d.iterdir())
    box.usr_d = box.tmp / "usr" / "share" / "limine-entry-tool.d"  # absent, as on 1.37.1
    ...
    box.machine_id = box.etc / "machine-id"
    box.machine_id.write_text(f"{MACHINE_ID}\n")
    box.modules = box.tmp / "usr" / "lib" / "modules"
    add_kernel(box, KERNEL_VERSION, "linux")
    box.efi_dir = box.tmp / "sys-firmware-efi"
    box.efi_dir.mkdir()
    ...
    box.effective_cmdline = ""  # empty: the stub assembles it from the box
    box.boot_images = [image_path(box, "linux")]
    box.env.update(
        {
            "_HYPRCONF_LIMINE_DEFAULT": str(box.limine),
            "_HYPRCONF_LIMINE_ENTRY_CONF": str(box.entry_conf),
            "_HYPRCONF_LIMINE_CONF_D": str(box.conf_d),
            "_HYPRCONF_LIMINE_USR_D": str(box.usr_d),
            ...
            "_HYPRCONF_MACHINE_ID": str(box.machine_id),
            "_HYPRCONF_MODULES_DIR": str(box.modules),
            "_HYPRCONF_EFI_DIR": str(box.efi_dir),
...
def run(
    ...
    """The tool against the box. The fakes' knobs are read here, so a test may
    set them at any point before the run; assume_tty reaches the prompts
    through the _HYPRCONF_ASSUME_TTY seam and stdin_text feeds their reads."""
    box.env.update(
        ...
        FAKE_EFFECTIVE_CMDLINE=box.effective_cmdline,
        FAKE_BOOT_IMAGES="\n".join(str(p) for p in box.boot_images),
    )
    return box.run(TOOL, *args, tty=assume_tty, stdin=stdin_text)


def add_kernel(box: Box, version: str, pkgbase: str) -> None:
    d = box.modules / version
    d.mkdir(parents=True)
    (d / "pkgbase").write_text(f"{pkgbase}\n")
    (d / "vmlinuz").write_bytes(b"")


def image_path(box: Box, name: str, *, uki: bool = True) -> Path:
    """Where limine-mkinitcpio-install puts the image (limine-entry-tool README)."""
    if uki:
        return box.esp / "EFI" / "Linux" / f"omarchy_{name}.efi"
    return box.esp / MACHINE_ID / name / "initramfs"


def write_stale_image(box: Box, path: Path | None = None, *, age: int = 3600) -> Path:
    """An image from an earlier build: present, but older than any run."""
    path = path or box.boot_images[0]
...
def drop_uki(box: Box) -> None:
    """ENABLE_UKI=no: the packaged /etc/limine-entry-tool.conf with no
    omarchy-uki.conf over it, so the image is the plain initramfs."""
    (box.conf_d / "omarchy-uki.conf").unlink()
    box.omarchy_conf_d = sorted(p.name for p in box.conf_d.iterdir())
    box.boot_images = [image_path(box, "linux", uki=False)]


def drop_uefi(box: Box) -> None:
    """A box that did not boot UEFI: ENABLE_UKI=yes cannot produce a UKI."""
    box.efi_dir.rmdir()
    box.boot_images = [image_path(box, "linux", uki=False)]
...
def unpadded(text: str) -> str:
    """Status pads its labels into columns: match on what it says and the path
    it names, not on the width of the gap."""
...
def test_every_external_the_tool_calls_has_a_fake(box: Box) -> None:
    ...
    call to it is drift, and turns this red."""
...
def test_restraint_scan() -> None:
    """This tool's own invariants: never the password slot, never sbctl (it
    signs nothing), never `set -e` (it handles its own failures) and never a
    .bak copy of a system file. The shebang, the `set -` line and the
    overridable seams are test_every_overlay_script_carries_the_documented_header's,
    the pacman and AUR forms test_overlay_never_uses_forbidden_pacman_or_aur_forms'
    — both scan this file too (test_omarchy_install.py)."""
    text = TOOL.read_text()
...
def test_enroll_happy_path(box: Box) -> None:
    ...
    # /etc/default/limine is read, never edited
    assert box.limine.read_text() == box.limine_file
    ...
    image = image_path(box, "linux")
    assert image.is_file()
    assert ran_as_root(box, f"stat -c %Y -- {image}")
...
READ_SHAPES = [
    ...
    # systemd-cryptsetup-generator(8)'s UUID-less global form, plausibly
    # hand-written: not a per-device competitor, so tolerated and left alone
    pytest.param(
        f'KERNEL_CMDLINE[default]+="{CRYPT} rd.luks.options=fido2-device=auto"',
        "root",
...
# ---------------------------------------------------------------------------
# disable and remove never read /etc/default/limine
# ---------------------------------------------------------------------------


def test_disable_and_remove_never_read_the_cmdline_line(box: Box) -> None:
    """Only enroll needs the file (the mapper name comes from it). disable and
    remove touch nothing but the two drop-ins, so a line there they could not
    read with certainty — or no KERNEL_CMDLINE[default] line at all — is not
    their business: no refusal, the file byte-identical."""
    assert run(box, "enroll", "--yes", "--device", DEV).returncode == 0
    ...
    set_limine_line(box, LIMINE_LINE)  # enroll itself still needs a readable line
    assert run(box, "enroll", "--yes", "--device", DEV).returncode == 0
    box.limine.write_text(f'ESP_PATH="{box.esp}"\n')  # no cmdline line at all
...
def test_rebuild_dies_when_no_image_is_where_limine_puts_it(box: Box) -> None:
    ...
    res = run(box, "enroll", "--yes", "--device", DEV)
    assert res.returncode != 0
    assert f"no boot image at {image_path(box, 'linux')}" in res.stderr
...
def test_rebuild_checks_every_installed_kernel(box: Box) -> None:
    ...
    res = run(box, "enroll", "--yes", "--device", DEV)
    assert res.returncode == 0, res.stderr
    assert f"{lts}: rebuilt" in res.stdout and f"{image_path(box, 'linux')}: rebuilt" in res.stdout


@pytest.mark.parametrize(
    "drop",
    [pytest.param(drop_uki, id="ENABLE_UKI=no"), pytest.param(drop_uefi, id="no-UEFI")],
)
def test_rebuild_looks_for_the_initramfs_without_uki(box: Box, drop) -> None:
    ...
    drop(box)
    initramfs = image_path(box, "linux", uki=False)
    assert box.boot_images == [initramfs]
    res = run(box, "enroll", "--yes", "--device", DEV)
    assert res.returncode == 0, res.stderr
    assert f"{initramfs}: rebuilt" in res.stdout
    assert ran_as_root(box, f"stat -c %Y -- {initramfs}")
...
def test_the_non_latin_layout_list_is_the_tools_own() -> None:
    ...
    /etc/mkinitcpio.conf.d/omarchy_hooks.conf, is not under $OMARCHY_PATH and
    the suites must pass with no Omarchy at all — so the copy above is pinned
    against the tool's case line here, whole, instead of one subprocess per
    layout below."""
...
def test_status_prints_facts(box: Box) -> None:
    ...
    assert f"KERNEL_CMDLINE[default]+= line found in {box.limine}" in out
    ...
    assert "KERNEL_CMDLINE[default]+= line found" in res.stdout
...
# order, and the interactive paths
# ---------------------------------------------------------------------------
...
def test_disable_after_a_failed_enroll_rebuild_verifies_against_stock(box: Box) -> None:
    ...
    box.limine_mkinitcpio = "fail"
    assert run(box, "enroll", "--yes", "--device", DEV).returncode == 1
    assert box.dropin.exists() and box.limine_dropin.exists()
    write_stale_image(box)  # the ESP still holds the pre-enroll image
...
def test_disable_removes_the_verified_record(box: Box) -> None:
    ...
    state = box.mkinitcpio_d / f".{DROPIN_NAME}.verified"
    assert state.exists()
    assert run(box, "disable", "--yes").returncode == 0
...
def test_interactive_menu_rejects_an_out_of_range_choice(box: Box) -> None:
    box.lsblk = LSBLK_STOCK + f"/dev/sdd crypto_LUKS 2 {UUID2}\n"
    res = run(box, "enroll", assume_tty=True, stdin_text="9\n")
