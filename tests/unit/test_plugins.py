...
def test_resources_feeders_restart_on_a_capped_backoff() -> None:
    """A dead feeder is restarted on a capped backoff, never a flat retry
    (why, and the ~78,000 execs a day that motivated it: Service.qml's own
    header). The shape is pinned rather than the behaviour, which would need
    a running shell; it was verified under quickshell 0.3.1 against a feeder
    that exits at once — 7 execs in 63 s, then nothing — and against one that
    emits a line every time: a steady 1 s."""
...
def test_clock_plugin_tracks_omarchys_stock_clock() -> None:
    ...
    plus a header comment and exactly the three deltas the diff below lists —
    seconds precision, the panel loaded from the running Omarchy, the module
    id forwarded to it — and nothing else. An Omarchy release that changes its
    clock turns this red on a box with that release, and the header carries
    the refresh recipe. Skips without an installed Omarchy; the deltas
    themselves are pinned in CI by the install suite's clock stage test."""
