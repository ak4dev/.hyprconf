...
def test_light_theme_gets_the_light_prefs_and_retires_the_old_ones(
    hermetic: dict[str, Path],
) -> None:
    ...
    assert "browser.theme" not in js and 'user_pref("keep.me", 1);' in js
...
def test_template_renders_the_firefox_variables_and_selectors() -> None:
    ...
    render every variable and selector the retired Python emitted."""
...
def test_theme_set_hook_runs_the_firefox_bridge_and_only_that(
    ...
    """VS Code is Omarchy's own now (omarchy-install-editor-vscode installs
    it, omarchy-theme-set-vscode themes it in omarchy-theme-set's fan-out),
    so the hook no longer sources Omarchy's script or knows Code - OSS. It
    must not abort a theme switch (no set -e) and must never escalate."""
    code = "\n".join(
        line for line in HOOK.read_text().splitlines() if not line.lstrip().startswith("#")
    )
    for forbidden in ("set -e", "sudo", "source ", "set_theme", "vscode", "code", "Code - OSS"):
