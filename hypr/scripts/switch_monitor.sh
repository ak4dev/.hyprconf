...
# that stops every require after it (input, bindings, looknfeel, autostart).
SOURCE="$CONFIG_DIR/pcMonitors.$1.lua"
[[ -f "$SOURCE" ]] || SOURCE="$CONFIG_DIR/${1}Monitors.lua"
if [[ ! -f "$SOURCE" && -f "$CONFIG_DIR/pcMonitors.$1" ]]; then
    log_die "pcMonitors.$1 is a hyprlang preset; convert it to pcMonitors.$1.lua (hl.monitor) first"
fi
...
# preset switch used to enable the right outputs while workspaces 1-6 stayed
# where the last layout had them. Walk the preset's own workspace rules and
...
while IFS=$'\t' read -r ws mon; do
    hyprctl dispatch "hl.dsp.workspace.move({ workspace = $ws, monitor = \"$mon\" })" \
        >/dev/null 2>&1 || true
done < <(sed -nE 's/.*hl\.workspace_rule\(\{ *workspace *= *"([0-9]+)".*monitor *= *"([^"]+)".*/\1\t\2/p' "$SOURCE")
