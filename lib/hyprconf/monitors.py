...
class MonitorConfig:
    ...
    def to_line(self) -> str:
        ...
        return monitor_call(self.name, self.resolution, self.position, self.scale, self.extras)
...
def monitor_call(name: str, resolution: str, position: str, scale: str, extras: str = "") -> str:
    """Build the ``hl.monitor({ … })`` statement for one output.

    One string serves both consumers: it is the line persisted to
    ``monitors.lua`` and the chunk :func:`hyprconf.hyprctl.apply_monitor`
    hands to ``hyprctl eval`` for the live change, so the two cannot drift.
    """
    table = _fields_to_table(name, resolution, position, scale, extras)
...
def upsert_monitor(
    ...
    line = monitor_call(name, resolution, position, scale, extras)
