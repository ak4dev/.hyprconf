...
# offered for removal here, EXCEPT the yay helper itself, which is kept so
# the user can still build AUR packages manually when they choose to.
# Removal is destructive, so it always prompts and
...
force_stow_package() {
    ...
    if ! stow -d "$stow_dir" -t "$target_dir" --no-folding --ignore='__pycache__' --"$stow_mode" "$package" 2>/dev/null; then
        if [[ "$stow_mode" == "stow" ]]; then
            ...
            log_warn "Conflicts in $package — linking new files individually (user files preserved). Run 'setup.sh --sync --full' to reset to defaults."
...
setup_power_monitor() {
    ...
    # user-facing copy for manual hyprconf-power-monitor runs only.
            ...
            (
                ...
                if _gpu_load_config 2>/dev/null && _gpu_has_other_nvidia_gpu 2>/dev/null; then
                    ...
                    _gpu_configure_boot_binding 2>/dev/null \
                        ...
                        || log_warn "GPU boot-time binding sync failed — run gpu-passthrough.sh setup manually."
                fi
            ) || true
        fi
    fi
...
reapply_current_theme() {
    ...
    python3 "$script" "$theme_name" --no-reload \
        ...
        || log_warn "Theme apply failed — run switch_theme.py manually."
}
...
stow_all_packages() {
    ...
    # ~/.local/bin/hyprconf unlinked.  Re-link it so the TUI launcher stays
    ...
    if [[ -f "$_hc_src" && ! -e "$_hc_dst" ]]; then
        ...
        log_warn "hyprconf launcher re-linked as fallback — re-run setup.sh to fully restore."
    fi

    detect_gpu_and_link_monitor_config

    # The hypr package ships systemd user units (hyprlock.service): make a
    # live user manager re-read unit files so changes from this run take
    # effect without a re-login. No-op in chroot (no live systemd); on first
    # install the units are scanned at session start anyway.
    _in_chroot || systemctl --user daemon-reload 2>/dev/null || true

    if [[ ${#_stow_failures[@]} -gt 0 ]]; then
        log_warn "Stow failed for: ${_stow_failures[*]} — re-run setup.sh to fix."
        return 1
    fi
