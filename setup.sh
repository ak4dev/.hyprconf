...
write_hardware_conf() {
    ...
    local has_touch=false has_accel=false has_kbd=false

    _has_touchscreen   && has_touch=true
    _has_accelerometer && has_accel=true
    _has_physical_keyboard && has_kbd=true

    {
        ...
        if $has_touch && ! $has_kbd; then
            printf '\n# Touch-only control panel (no physical keyboard detected)\n'
            printf 'exec-once = touch-panel\n'
        fi
    ...
    if $has_touch || $has_accel; then
        log_ok "Hardware conf written → $conf_file"
    else
        log_ok "No touchscreen or accelerometer detected — hardware conf cleared."
    fi
}
...
setup_hardware_features() {
    ...
    if _has_touchscreen; then
        ...
        if ! _has_physical_keyboard; then
            log_step "Installing gtk-layer-shell (touch-only device)..."
