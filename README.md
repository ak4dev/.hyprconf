...
hyprconf-brightness        Backlight up/down/get with OSD feedback
...
| `XF86MonBrightnessUp / Down` | Brightness ±5% via `hyprconf-brightness` — perceptual curve, floored at 2% so the panel never goes black, shown on the OSD, and a no-op on machines with no backlight |
