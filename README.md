...
Every widget is *enabled* once, and the clock's format and anchor are set once — disabling any of them sticks. `modules/bar-active-window`, `modules/bar-clock` and `modules/bar-workspaces` **symlink** their folders into `~/.config/omarchy/plugins/` and ask the shell to rescan on every run, so a `git pull` is the update (the shell's own watch does not follow a link). `plugins/hyprconf-resources`, the last one still shipped from `plugins/`, is re-synced on every run, so a `git pull` updates it — the comparison is bytes **and** file modes, so a feeder that lost its exec bit (a restore of `~/.config` without permissions) is put back too — staged in a sibling temp dir and moved into place, the way `omarchy plugin clone` lands a copy, then `omarchy-shell shell rescanPlugins` hot-reloads it. `omarchy bar set hyprconf.clock format 'HH:mm'` reformats the clock.
