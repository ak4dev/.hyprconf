...
| clock | `hyprconf.clock`: Omarchy's own clock widget ticking seconds (`plugins/hyprconf-clock/`, a `clonedFrom` copy with two deltas), format `hh:mm:ss AP` — the copy synced on every run; the enable, the format and the anchor **set once** | `plugins/hyprconf-clock/` synced into `~/.config/omarchy/plugins/` like the other three; then, once: `omarchy-plugin-enable` (the shell swaps it into `omarchy.clock`'s slot), `omarchy-bar set`, and the bar's `centerAnchor` follows only if it still pointed at `omarchy.clock` |
...
| Piece | What |
|---|---|
| `themed/userChrome.css.tpl` → `~/.config/omarchy/themed/` | A user template: Firefox's `--lwt-*`/`--toolbar-*` theme variables **and** direct rules for the toolbox, tabs, URL bar and sidebar (on Firefox 154 the variables are inert — see the hook row — and the direct rules are what paint the chrome), from `{{ background }}`, `{{ foreground }}`, `{{ accent }}`, `{{ dark_background }}`, `{{ lighter_background }}`, plus `--hyprconf-theme-mode: {{ mode }}` — the mode Omarchy resolved (`mode = "light"` in the theme's `colors.toml`, the legacy `light.mode` file, else dark). `omarchy-theme-set-templates` renders it on every theme set into `~/.local/state/omarchy/current/theme/userChrome.css` |
...
- runs `limine-mkinitcpio`, which rebuilds every initramfs/UKI and the Limine entries — the same rebuild Omarchy's own `omarchy-hibernation-setup` runs after changing its drop-in (`limine-update` would also re-deploy the bootloader binary and build a second time; Omarchy uses it only to reset `/boot/limine.conf`).
