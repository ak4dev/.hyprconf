...
Config stored at `~/.config/hyprconf/gpu-passthrough.conf` (GPU) and `~/.config/hyprconf/gpu-vm.conf` (VM). Doctor checks IOMMU, VFIO modules, and user groups when the `vfio` addon is installed.
